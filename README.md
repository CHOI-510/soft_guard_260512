...
| 항목 | 버전 | 설치 상태 |
|------|------|---------|
| Python | 3.13+ | ✅ 가상환경에 포함 |
| Google Gemini API Key | - | ⚠️ 직접 설정 필요 |
| 인터넷 연결 | - | ✅ 필수 |
...
모두 `soft_guard` 가상환경에 설치되어 있습니다:

```
google-genai==0.8.6+      # Google AI API
requests==2.34.0+         # HTTP 통신
(json, os, time, datetime) # 표준 라이브러리
```
