...
또는 전체 파이프라인을 한 번에 실행하는 `softguard_loop.py`를 사용하면 24시간 자동 대기/처리 모드로 운영할 수 있습니다.
...
├── 📂 test/                    # 테스트 파일들
│
├── 📜 softguard_loop.py        # 24시간 자동화 통합 파이프라인
├── 📜 softguard_pipeline.py    # 파이프라인 모듈
...
```powershell
# 방법 2: 통합 자동 실행 (추천 - 24시간 대기/처리)
python softguard_loop.py
```
...
**설명:**
- `softguard_loop.py`는 `1_download_video.py`, `2_analyze_video.py`, `3_upload_result.py`를 한 번에 연결하여 무한 루프 형태로 실행합니다.
- 대기 중인 영상이 없으면 60초마다 서버를 재확인하고, 처리 후에는 5초 뒤에 즉시 다시 확인합니다.
- 영상 다운로드, AI 분석, 결과 업로드까지 모두 자동으로 처리하며, 각 단계의 상태는 `workflow_log.json`에 기록됩니다.

**특징:**
- ✅ 24시간 자동 대기/처리 모드
- ✅ 각 단계 실패 시 재시도 및 오류 로그 기록
- ✅ `KeyboardInterrupt` (`Ctrl+C`) 입력 시 안전하게 종료
- ✅ 이미 다운로드된 영상은 재다운로드 없이 건너뜀
...
```
==================================================
🚀 SoftGuard 24시간 자동화 파이프라인 가동 시작!
==================================================
📡 [Step 1] 서버에 대기 중인 영상이 있는지 확인합니다...
🎯 타겟 영상 파일명: The.mp4
📥 다운로드 주소: https://softguard-ah9q.onrender.com/api/videos/download/The.mp4
✅ 다운로드 완료! 저장 위치: D:\jeong\soft_guard\video\The.mp4
🧠 [Step 2] AI 영상 분석 시작: The.mp4
🎬 영상 업로드 중: D:\jeong\soft_guard\video\The.mp4
......
✅ 분석 결과 저장 완료: result_The_00.json
📤 [Step 3] 분석 결과 서버 업로드 시작: result_The_00.json
