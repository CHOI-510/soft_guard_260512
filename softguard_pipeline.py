...
def step1_download_video():
    ...
    try:
        ...
        # ⭐ [수정됨] 파싱할 때 eventId도 같이 뽑아옵니다.
        def parse_pending(item):
            event_id = None
            if isinstance(item, dict):
                video_url = item.get('videoUrl', '')
                event_id = item.get('eventId') # ✅ 여기서 추출!
...
def step3_upload_result(json_filename):
    ...
    for attempt in range(MAX_RETRIES):
        try:
            ...
            # ⭐ [수정됨] 장부에서 eventId를 꺼내서 metadata 안에 쏙 집어넣기!
...
# ==========================================
def main():
    parser = argparse.ArgumentParser(description="SoftGuard 통합 파이프라인 (다운로드 -> 분석 -> 업로드)")
    parser.add_argument("--skip-download", action="store_true", help="다운로드 단계를 생략합니다.")
    parser.add_argument("--skip-analyze", action="store_true", help="AI 분석 단계를 생략합니다.")
    parser.add_argument("--skip-upload", action="store_true", help="결과 업로드 단계를 생략합니다.")
    args = parser.parse_args()

    print("==================================================")
    print("🚀 SoftGuard One-Click 파이프라인 가동 시작!")
    print("==================================================")
    ...
    if not args.skip_download:
        ...
        if not video_filename:
            print("🛑 처리할 새 영상이 없거나 통신에 실패했습니다. 작업을 종료합니다.")
            return
    else:
        video_filename = get_workflow_value("download", "downloaded_video")
        print(f"⏩ 다운로드 스킵 됨. 기존 영상 사용: {video_filename}")

    # 2. 분석
    json_filename = None
    if not args.skip_analyze:
        if not video_filename:
            print("🛑 분석할 영상 파일명이 없어 작업을 중단합니다.")
            return
        json_filename = step2_analyze_video(video_filename)
        if not json_filename:
            print("🛑 AI 분석 중 오류가 발생하여 작업을 종료합니다.")
            return
    else:
        json_filename = get_workflow_value("analyze", "output_result")
        print(f"⏩ 분석 스킵 됨. 기존 결과 사용: {json_filename}")

    # 3. 업로드
    if not args.skip_upload:
        if not json_filename:
            print("🛑 업로드할 결과 파일(JSON)이 없어 작업을 중단합니다.")
            return
